...
typedef union { double d; float64_t u; } f64_u;
typedef union { float      f; float32_t u; } f32_u;

static inline float64_t d2u(double d)   { f64_u u; u.d = d; return u.u; }
static inline double    u2d(float64_t u){ f64_u v; v.u = u; return v.d; }
static inline float32_t f2u(float f)    { f32_u u; u.f = f; return u.u; }
static inline float     u2f(float32_t u){ f32_u v; v.u = u; return v.f; }
...
double __adddf3(double a, double b) { return u2d(f64_add(d2u(a), d2u(b))); }
double __subdf3(double a, double b) { return u2d(f64_sub(d2u(a), d2u(b))); }
double __muldf3(double a, double b) { return u2d(f64_mul(d2u(a), d2u(b))); }
double __divdf3(double a, double b) { return u2d(f64_div(d2u(a), d2u(b))); }
...
int __ltdf2(double a, double b) {
    ...
    if (f64_lt(ua, ub)) return -1;
    if (f64_eq(ua, ub)) return 0;
    return 1;
}
...
int __gtdf2(double a, double b) { return __ltdf2(b, a); }
int __gedf2(double a, double b) { return __ledf2(b, a); }
int __eqdf2(double a, double b) { return f64_eq(d2u(a), d2u(b)) ? 0 : 1; }
int __nedf2(double a, double b) { return f64_eq(d2u(a), d2u(b)) ? 0 : 1; }

/* -------------------------------------------------------------------------
 * Double ↔ integer conversions
 * ------------------------------------------------------------------------- */

int          __fixdfsi(double a) { return (int)f64_to_i32_r_minMag(d2u(a), 0); }
long long    __fixdfdi(double a) { return f64_to_i64_r_minMag(d2u(a), 0);      }
float        __truncdfsf2(double a) { return u2f(f64_to_f32(d2u(a)));           }

double __floatsidf(int a)          { return u2d(i32_to_f64(a));  }
double __floatdidf(long long a)    { return u2d(i64_to_f64(a));  }
double __floatunsidf(unsigned int a){ return u2d(ui32_to_f64(a)); }

/* float → double */
double __extendsfdf2(float a)  { return u2d(f32_to_f64(f2u(a)));  }

/* int64 → float (single precision) */
float __floatdisf(long long a) { return u2f(f64_to_f32(i64_to_f64(a))); }
...
int __gttf2(const float128_t *a, const float128_t *b) { return __lttf2(b, a); }
int __getf2(const float128_t *a, const float128_t *b) { return __letf2(b, a); }
...
/* quad → int */
int          __fixtfsi(const float128_t *a) { return (int)f128_to_i32_r_minMag(*a, 0);  }
long long    __fixtfdi(const float128_t *a) { return f128_to_i64_r_minMag(*a, 0);        }
unsigned int __fixunstfsi(const float128_t *a) { return f128_to_ui32_r_minMag(*a, 0);    }

/* int → quad */
void __floatsitf(float128_t *r, int a)           { *r = i32_to_f128(a);  }
void __floatditf(float128_t *r, long long a)      { *r = i64_to_f128(a);  }
void __floatunsitf(float128_t *r, unsigned int a) { *r = ui32_to_f128(a); }

/* float/double → quad */
void __extendsftf2(float128_t *r, float a)  { *r = f32_to_f128(f2u(a));  }
void __extenddftf2(float128_t *r, double a) { *r = f64_to_f128(d2u(a));  }

/* quad → float/double */
float  __trunctfsf2(const float128_t *a) { return u2f(f128_to_f32(*a)); }
double __trunctfdf2(const float128_t *a) { return u2d(f128_to_f64(*a)); }
...
int wctomb(char *s, unsigned int wc) { (void)s; (void)wc; return 0; }
