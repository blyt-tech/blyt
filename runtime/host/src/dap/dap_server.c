...
static int q_empty(void) { return g_dap.q_head == g_dap.q_tail; }
static int q_full(void)  { return ((g_dap.q_tail + 1) % MSG_QUEUE_CAP) == g_dap.q_head; }
...
int fc_dap_host_recv(char *buf, size_t max_len) {
    pthread_mutex_lock(&g_dap.mu);
    while (g_dap.running && g_dap.client_fd >= 0 &&
           !g_dap.continue_pending && q_empty()) {
        pthread_cond_wait(&g_dap.msg_cond, &g_dap.mu);
    }
