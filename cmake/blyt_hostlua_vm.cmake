  ...
  elseif(NOT musl_SOURCE_DIR OR NOT EXISTS "${musl_SOURCE_DIR}/src/math/sin.c")
    ...
    if(_BHLV_MUSL_ARCH)
      ...
      if(NOT _BHLV_ALLTYPES_RC EQUAL 0)
        ...
        set(_BHLV_FLOAT_H "${_BHLV_DIR}/bits/float.h")
        file(
          WRITE "${_BHLV_FLOAT_H}"
...
function(blyt_add_hostlua_vm NAME CONTRACT_FLAG)
  ...
  # Force-include the generated <bits/float.h> so ccache tracks it correctly
  # (blyt#229). It sits in ${_BHLV_DIR} to shadow musl's arch bits/float.h, which
  # is on a *later* -I dir above. ccache direct mode records the header set a TU
  # resolved during its *first* compile; if these math/conv objects were ever
  # cached before this generated shadow existed, the manifest lists musl's arch
  # float.h and a rebuild returns that stale object — it never re-preprocesses to
  # notice the new shadow (blyt#225 Phase B: a stale frexpl.c.o kept the 113-bit
  # long-double path and self-recursed into a stack overflow on macOS). Naming the
  # shadow on the command line makes it a real ccache dependency: the added flag
  # changes direct mode's primary key (busting any stale entry) and the header is
  # recorded/rehashed on every build. The include guard means the later
  # #include <bits/float.h> is a no-op, so this is behaviourally identical to the
  # -I shadow alone.
  target_compile_options(${_fpm} PRIVATE "-include" "${_BHLV_FLOAT_H}")
  ...
  # Same ccache shadow-header dependency fix as the fpm lib above (blyt#229): the
  # VM TU has ${_BHLV_DIR} on its -I path too, so name the generated shadow
  # <bits/float.h> on the command line to keep it a tracked ccache dependency.
  target_compile_options(${NAME} PRIVATE "-include" "${_BHLV_FLOAT_H}")
