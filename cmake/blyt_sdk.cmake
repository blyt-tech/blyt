...
if(FOUND_LLDB_DAP)
  # blyt-lldb-dap: on Unix, a thin wrapper that disables lldb's debuginfod symbol
  # locator for every native (lldb-dap → GDB stub) debug session. LLVM on Ubuntu
  # ships plugin.symbol-locator.debuginfod.server-urls defaulting to
  # https://debuginfod.ubuntu.com/ with external lookup on (and an empty
  # DEBUGINFOD_URLS env does NOT override it), so lldb blocks ~90s per loaded
  # object — the stub ELF and the cart .so, whose build-ids are on no server —
  # i.e. ~180s of dead time before the first stop. Our carts embed their own
  # DWARF; that lookup only ever times out. --pre-init-command runs the setting
  # right after the debugger is created and before the target, so no lookup is
  # attempted. Wrapping blyt-lldb-dap (rather than each launch config) is the one
  # place every caller — the VS Code extension, the cargo lldb-dap/dap suites,
  # and the CLI — goes through.
  #
  # Windows keeps the plain link: a POSIX-sh wrapper won't run there, and the
  # debuginfod default is an Ubuntu-LLVM packaging quirk absent on Windows. A
  # Windows-native wrapper is tracked in #324.
  if(CMAKE_HOST_WIN32)
    file(CREATE_LINK "${FOUND_LLDB_DAP}" "${SDK_BIN}/blyt-lldb-dap" SYMBOLIC)
  else()
    file(
      WRITE "${SDK_BIN}/blyt-lldb-dap"
      "#!/bin/sh\n"
      "# Generated by blyt_sdk.cmake — disables lldb's debuginfod locator\n"
      "# (~180s of dead network lookups per native debug session). See there.\n"
      "exec \"${FOUND_LLDB_DAP}\" \\\n"
      "  --pre-init-command 'settings set plugin.symbol-locator.debuginfod.server-urls \"\"' \\\n"
      "  \"$@\"\n")
    file(
      CHMOD
      "${SDK_BIN}/blyt-lldb-dap"
      PERMISSIONS
      OWNER_READ
      OWNER_WRITE
      OWNER_EXECUTE
