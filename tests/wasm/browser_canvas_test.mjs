...
if (!htmlPath || !goldenPath) {
    process.stderr.write('usage: browser_canvas_test.mjs <html_path> <golden_bin_path>\n');
    process.exit(1);
}
...
if (goldenBin.length !== W * H * 4) {
    process.stderr.write(`golden size mismatch: expected ${W * H * 4} bytes, got ${goldenBin.length}\n`);
    process.exit(1);
}
...
try {
    ...
    await page.addInitScript(() => {
        ...
        CanvasRenderingContext2D.prototype.putImageData = function (imageData, ...rest) {
    ...
    await page.waitForFunction(
        () => Array.isArray(window.__blyt_frame0_rgba),
        { timeout: TIMEOUT_MS }
    );
    ...
    if (mismatches > 0) {
        process.stderr.write(`FAIL: ${mismatches} of ${npixels} pixels differ from golden\n`);
        process.exit(1);
    }
    ...
    if (e.message.includes('Executable doesn\'t exist') || e.message.includes('browserType.launch')) {
        process.stderr.write('Hint: run `npx playwright install chromium` to download the browser.\n');
    }
    process.exit(1);
}
