#!/usr/bin/env node
/*
 * tests/gdb/run_libretro_gdb_test.mjs — libretro GDB test orchestrator.
 *
 * Spawns blyt-libretro-runner with BLYT_GDB_PORT=0, waits for the GDB TCP
 * port to appear on stdout or stderr ("blyt: GDB listening on port N"), then
 * runs gdb_test.mjs in TCP mode against that port.
 *
 * Usage:
 *   node run_libretro_gdb_test.mjs <runner_path> <blyt_libretro.so> <cart_path>
 *
 * Environment:
...
if (!RUNNER || !SO_PATH || !CART) {
    process.stderr.write(
        'usage: run_libretro_gdb_test.mjs <runner> <blyt_libretro.so> <cart>\n'
    );
    process.exit(1);
}
...
async function main() {
    ...
    console.log(`[libretro_gdb_test] runner GDB on tcp://127.0.0.1:${port}`);
    ...
    /* gdb_test sent vCont;c; wait for the runner to exit. */
...
main().then(() => process.exit(0)).catch((e) => {
    console.error('[libretro_gdb_test] FAILED:', e.message);
    process.exit(1);
});
