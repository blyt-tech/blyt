...
if (!BLYTRUN || !CART) {
	process.stderr.write(
		'usage: run_sdl_interrupt_test.mjs <blytplay> <cart>\n',
	);
	process.exit(1);
}
...
async function main() {
	...
	try {
		...
		console.log(
			'[interrupt_test] sent vCont;c, waiting 100ms then interrupting…',
		);
		...
		const stop = await Promise.race([
			...
			new Promise((_, rej) =>
				setTimeout(
					() => rej(new Error('timeout waiting for T02')),
					30000,
				),
			),
		]);
		...
		if (!stop?.startsWith('T02')) {
			process.stderr.write(
				`[interrupt_test] FAIL: expected T02, got: ${stop}\n`,
			);
			process.exit(1);
		}
