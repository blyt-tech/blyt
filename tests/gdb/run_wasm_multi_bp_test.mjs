...
if (!WASM_DIR || !CART_PATH) {
	process.stderr.write(
		'usage: run_wasm_multi_bp_test.mjs <wasm_dir> <cart_path>\n',
	);
	process.exit(1);
}
...
function startRelay() {
	return new Promise((resolvePort) => {
		...
		function makeSide(socket, label, onClose) {
			...
			socket.on('data', (chunk) => {
				...
				side.buf = wsParseFrames(side.buf, (text) => {
					if (text === null) {
						process.stderr.write(
							`[relay] ${label} close frame received\n`,
						);
						if (!side.closed) {
					...
					process.stderr.write(
						`[relay] ${label}→other: ${JSON.stringify(text.slice(0, 80))}\n`,
					);
			...
			socket.on('error', (e) => {
				process.stderr.write(
					`[relay] ${label} socket error: ${e.message}\n`,
				);
				side.closed = true;
				if (onClose) onClose();
			});
		...
		server.on('upgrade', (req, socket) => {
			...
			if (sideA === null) {
				sideA = makeSide(socket, 'sideA', () => {
					process.stderr.write(
						'[relay] sideA closed — destroying sideB\n',
					);
		...
		server.listen(0, '127.0.0.1', () =>
			resolvePort({ port: server.address().port, waitForRuntime }),
		);
	});
}
...
async function main() {
	...
	console.log(
		`[run_wasm_multi_bp_test] relay on ws://127.0.0.1:${relayPort}/gdb`,
	);

	const runtimeDone = loadWasmRuntime(WASM_DIR, CART_PATH, relayPort).catch(
		(e) => {
			console.error(
				'[run_wasm_multi_bp_test] WASM runtime error:',
				e.message,
			);
		},
	);
