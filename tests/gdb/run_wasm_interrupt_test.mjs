...
if (!WASM_DIR || !CART_PATH) {
	process.stderr.write(
		'usage: run_wasm_interrupt_test.mjs <wasm_dir> <cart_path>\n',
	);
	process.exit(1);
}
...
function startRelay() {
	return new Promise((resolvePort) => {
		...
		const _clientSide = null; /* inline GDB client (this process) */
		...
		function makeSide(socket, label, onMsg, onClose) {
			...
			socket.on('data', (chunk) => {
				...
				side.buf = wsParseFrames(side.buf, (text) => {
					...
					process.stderr.write(
						`[relay] ${label}→client: ${JSON.stringify(text.slice(0, 80))}\n`,
					);
					if (onMsg) onMsg(text);
				});
			});
			...
			new Promise((res, rej) => {
				...
				const t = setTimeout(
					() =>
						rej(
							new Error(
								'WASM GDB runtime did not connect within timeout',
							),
						),
					timeoutMs,
				);
...
async function main() {
	...
	console.log(
		`[wasm_interrupt_test] relay on ws://127.0.0.1:${relayPort}/gdb`,
	);
	...
	console.log(
		'[wasm_interrupt_test] sent vCont;c, waiting 100ms then interrupting…',
	);
	...
	const stop = await Promise.race([
		client.recv(),
		new Promise((_, rej) =>
			setTimeout(() => rej(new Error('timeout waiting for T02')), 30000),
		),
	]);

	if (!stop?.startsWith('T02')) {
		process.stderr.write(
			`[wasm_interrupt_test] FAIL: expected T02, got: ${stop}\n`,
		);
		process.exit(1);
	}
