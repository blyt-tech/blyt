...
async function connect(endpointStr) {
    ...
    if (endpointStr.startsWith('tcp://')) {
        ...
        sock.on('error', (e) => process.stderr.write(`[multi_bp] tcp error: ${e.message}\n`));
        ...
        ws.addEventListener('error', (e) => process.stderr.write(`[multi_bp] ws error: ${e}\n`));
...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < bpAddrs.length; i++) {
            ...
            if (!stop || !stop.startsWith('T05')) {
                process.stderr.write(`[multi_bp] FAIL: expected T05 at bp ${i + 1}, got: ${stop}\n`);
                process.exit(1);
            }
