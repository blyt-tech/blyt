...
if (!WASM_DIR || !CART_PATH) {
    process.stderr.write('usage: run_wasm_hybrid_test.mjs <wasm_dir> <cart_path>\n');
    process.exit(1);
}
...
function startRelays() {
    return new Promise((resolve) => {
        ...
        const wsPromises = {
            '/dap': new Promise((r) => { wsResolvers['/dap'] = r; }),
            '/gdb': new Promise((r) => { wsResolvers['/gdb'] = r; }),
        };
        ...
        httpServer.on('upgrade', (req, socket) => {
            const p = req.url;
            if (p !== '/dap' && p !== '/gdb') { socket.destroy(); return; }
            ...
            socket.on('data', (chunk) => {
                ...
                side.buf = wsParseFrames(side.buf, (text) => {
                    ...
                    else if (p === '/dap' && text !== null) pendingDap.push(text);
                    else if (p === '/gdb' && text !== null) pendingGdb.push(text);
                });
            });
        ...
        let started = 0;
        let wsPort = 0, dapTcpPort = 0, gdbTcpPort = 0;
        ...
        httpServer.listen(0, '127.0.0.1', () => { wsPort = httpServer.address().port; tryResolve(); });
        dapTcpServer.listen(0, '127.0.0.1', () => { dapTcpPort = dapTcpServer.address().port; tryResolve(); });
        gdbTcpServer.listen(0, '127.0.0.1', () => { gdbTcpPort = gdbTcpServer.address().port; tryResolve(); });
    });
}
