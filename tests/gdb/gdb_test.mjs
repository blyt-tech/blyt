...
if (!endpoint) {
    process.stderr.write('usage: gdb_test.mjs <endpoint> [--break-addr HEXADDR]\n');
    process.exit(1);
}
...
async function connect(endpointStr) {
    ...
    if (endpointStr.startsWith('tcp://')) {
        ...
        sock.on('error', (e) => process.stderr.write(`[gdb] tcp error: ${e.message}\n`));
        await new Promise((res, rej) => { sock.once('connect', res); sock.once('error', rej); });
        sendFn  = (s) => sock.write(s);
        closeFn = () => { if (!closed) sock.destroy(); };

    } else if (endpointStr.startsWith('ws://') || endpointStr.startsWith('wss://')) {
        const ws = new WebSocket(endpointStr); /* Node.js 22+ built-in */
        ws.addEventListener('message', (ev) => onData(ev.data));
        ws.addEventListener('close',   () => onClose());
        ws.addEventListener('error',   (e) => process.stderr.write(`[gdb] ws error: ${e}\n`));
        await new Promise((res, rej) => {
            ws.addEventListener('open',  res, { once: true });
            ws.addEventListener('error', rej, { once: true });
        });
        sendFn  = (s) => ws.send(s);
        closeFn = () => { if (!closed) ws.close(); };
...
async function main() {
    ...
    try {
        ...
        const supported = await t.exchange('qSupported:multiprocess+;qXfer:exec-file:read+');
        ...
        if (breakAddr) {
            ...
            if (bpResp !== 'OK') {
                process.stderr.write(`[gdb_test] FAIL: Z0 response: ${bpResp}\n`);
                process.exit(1);
            }
            ...
            if (!stopReply || !stopReply.startsWith('T05')) {
                process.stderr.write(`[gdb_test] FAIL: expected T05, got: ${stopReply}\n`);
                process.exit(1);
            }
            ...
            if (regs && regs.length >= 264) {
                ...
                if (pc === 0) {
                    process.stderr.write('[gdb_test] FAIL: PC is zero after stop\n');
                    process.exit(1);
                }
            ...
            if (!stepReply || !stepReply.startsWith('T05')) {
                process.stderr.write(`[gdb_test] FAIL: expected T05 after step, got: ${stepReply}\n`);
                process.exit(1);
            }
        ...
        process.stderr.write(`[gdb_test] recv resolved: ${JSON.stringify(final)}\n`);
