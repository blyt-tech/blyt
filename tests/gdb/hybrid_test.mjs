...
if (!DAP_ENDPOINT || !GDB_ENDPOINT) {
	process.stderr.write(
		'usage: hybrid_test.mjs <dap-endpoint> <gdb-endpoint> [--gdb-break-addr HEX]\n',
	);
	process.exit(1);
}
...
async function connectGdb(endpointStr) {
	...
	const pending = [];
	const received =
		[]; /* buffer for packets that arrive before recv() is called */
	let buf = '';
	let closed = false;
	...
	sock.on('error', (e) =>
		process.stderr.write(`[gdb] error: ${e.message}\n`),
	);
...
async function connectDap(endpointStr) {
		...
		if (msg.type === 'response') {
			...
			if (p) {
				...
				if (msg.success) p.resolve(msg.body || {});
					...
					p.reject(new Error(msg.message || `${msg.command} failed`));
			}
	...
	sock.on('data', (chunk) => {
		tcpBuf = drainMessages(Buffer.concat([tcpBuf, chunk]));
	});
	await new Promise((res, rej) => {
		sock.once('connect', res);
		sock.once('error', rej);
	...
	return {
		...
		nextEvent(name) {
			return new Promise((resolve, reject) => {
				const timer = setTimeout(
					() => reject(new Error(`timeout: event "${name}"`)),
					60000,
				);
				const check = (e) => {
					if (e.event === name) {
...
async function main() {
	...
	/* ── GDB handshake ────────────────────────────────────────────────────── */
	const supported = await gdb.exchange(
	...
	if (sb.breakpoints[0]) {
		assert(
			...
			'DAP breakpoint at line 3 verified',
		);
	}
	...
	assert(
		dapStop1.body.reason === 'breakpoint' ||
			dapStop1.body.reason === 'step',
	...
	assert(
		frames1.some((f) => f.line === 3),
		'DAP stopped at line 3',
	);
	...
	if (gdbBreakAddr) {
		...
		console.log(
			'[dap] next sent — waiting for GDB T05 inside native function',
		);

		/* ── GDB: receive T05 when breakpoint fires inside the C function ── */
		const gdbT05 = await gdb.recv();
		assert(
			gdbT05?.startsWith('T05'),
			`GDB T05 at native entry: "${gdbT05}"`,
		);
		...
		if (regs && regs.length >= 264) {
			...
			assert(
				pc !== 0,
				`GDB PC non-zero after breakpoint: 0x${pc.toString(16)}`,
			);
			console.log(`[gdb] PC = 0x${pc.toString(16)}`);
		} else {
			console.log(
				`[gdb] register reply length ${regs?.length} (expected ≥264)`,
			);
		}
		...
		assert(step1?.startsWith('T05'), `GDB step 1 T05: "${step1}"`);
		...
		assert(step2?.startsWith('T05'), `GDB step 2 T05: "${step2}"`);
		...
		assert(
			dapStop2.body.reason === 'step' ||
				dapStop2.body.reason === 'breakpoint',
		...
		assert(
			frames2.some((f) => f.line === 4),
			'DAP at line 4 after step over native call',
		);
