...
impl Task for CompileCTask {
    fn key(&self) -> &str { &self.key_str }
    fn label(&self) -> String {
        let rel = self.src.strip_prefix(&self.project_dir).unwrap_or(&self.src);
        format!("compile  {}", rel.display())
    }
    fn inputs(&self) -> Vec<TaskInput> {
        let stem = self.src.file_stem().and_then(OsStr::to_str).unwrap_or("unknown");
        ...
        for f in &self.debug_flags { v.push(TaskInput::Value(f.clone())); }
        for d in &self.extra_defines { v.push(TaskInput::Value(d.clone())); }
        v
    }
    ...
    fn run(&self) -> Result<(), BuildError> {
        ...
        compile_c(
            &self.clang, &self.src, &self.build_dir, &self.sdk_include,
            &inc_refs, &self.extra_defines, &self.debug_flags,
        )?;
        Ok(())
    }
}
...
impl Task for CompileCppTask {
    ...
    fn inputs(&self) -> Vec<TaskInput> {
        ...
        let depfile = self.build_dir.join(format!("{stem}.d"));
        let mut v = vec![TaskInput::File(self.src.clone())];
        for dep in parse_depfile(&depfile) { v.push(TaskInput::File(dep)); }
    ...
    fn run(&self) -> Result<(), BuildError> {
        let inc_refs: Vec<&Path> = self.extra_includes.iter().map(PathBuf::as_path).collect();
        compile_cpp(
            &self.clangpp, &self.src, &self.build_dir, &self.sdk_include,
            &self.libcxx_include, &inc_refs, &self.debug_flags,
        )?;
        Ok(())
    }
}
...
impl Task for CodegenTask {
    fn key(&self) -> &str { "codegen" }
    fn label(&self) -> String { "codegen  cart_state".to_string() }
    fn inputs(&self) -> Vec<TaskInput> {
    ...
    fn outputs(&self) -> Vec<PathBuf> { self.declared_outputs() }
    fn run(&self) -> Result<(), BuildError> {
        let cart_config = crate::config::read_cart_config(&self.project_dir).map_err(err)?;
        let cart_state = generate_cart_state(
            &self.project_dir, &self.build_dir, &cart_config, self.needs_c, self.needs_rust,
        )?;
        if self.is_lua {
            let glue_src = self.build_dir.join("__blyt_lua_glue.c");
            generate_lua_glue_c(&glue_src, cart_state.buffers_present, &cart_state.lua_c_snippet)?;
        }
        Ok(())
    }
}
...
impl Task for LuaBytecodeTask {
    fn key(&self) -> &str { "lua_bytecode" }
    fn label(&self) -> String { "luac     src/game/lua/".to_string() }
    fn inputs(&self) -> Vec<TaskInput> {
        let mut v: Vec<TaskInput> =
            self.lua_files.iter().map(|f| TaskInput::File(f.clone())).collect();
        v.push(TaskInput::Value(self.luac.clone()));
        v
    }
    fn outputs(&self) -> Vec<PathBuf> { vec![self.output.clone()] }
    fn run(&self) -> Result<(), BuildError> {
        let mut cmd = Command::new(&self.luac);
        cmd.arg("-o").arg(&self.output).arg("-P").arg(&self.project_dir);
        for f in &self.lua_files { cmd.arg(f); }
        let status = cmd
            .status()
            .map_err(|e| err(format!("failed to run {}: {e}", self.luac)))?;
        if !status.success() { return Err(err("luac compilation failed")); }
        Ok(())
    }
}
...
impl Task for GenerateLuaDataTask {
    fn key(&self) -> &str { "gen_lua_data" }
    fn label(&self) -> String { "gen      cart_lua_data.c".to_string() }
    fn inputs(&self) -> Vec<TaskInput> { vec![TaskInput::File(self.bytecode_path.clone())] }
    fn outputs(&self) -> Vec<PathBuf> { vec![self.output_c.clone()] }
    fn run(&self) -> Result<(), BuildError> {
...
impl Task for ArchiveTask {
    ...
    fn inputs(&self) -> Vec<TaskInput> {
        let mut v: Vec<TaskInput> =
            self.obj_files.iter().map(|f| TaskInput::File(f.clone())).collect();
...
impl Task for CargoTask {
    fn key(&self) -> &str { &self.key_str }
    fn label(&self) -> String { self.label_str.clone() }
    fn inputs(&self) -> Vec<TaskInput> {
        ...
        if self.source_dir.is_dir() {
            if let Ok(mut srcs) = collect_rust_files(&self.source_dir) {
                srcs.sort();
                for s in srcs { v.push(TaskInput::File(s)); }
            }
        }
...
impl Task for ExternalCompileTask {
    ...
    fn inputs(&self) -> Vec<TaskInput> {
        let mut v: Vec<TaskInput> =
            self.src_files.iter().map(|f| TaskInput::File(f.clone())).collect();
...
impl Task for LinkTask {
    fn key(&self) -> &str { "link" }
    fn label(&self) -> String { "link     cart.elf".to_string() }
    fn inputs(&self) -> Vec<TaskInput> {
        let mut v: Vec<TaskInput> =
            self.obj_files.iter().map(|f| TaskInput::File(f.clone())).collect();
        if let Some(ref a) = self.rust_archive { v.push(TaskInput::File(a.clone())); }
        for a in &self.lib_archives { v.push(TaskInput::File(a.clone())); }
...
impl Task for FinaliseTask {
    fn key(&self) -> &str { "finalise" }
    fn label(&self) -> String { "finalise cart".to_string() }
    fn inputs(&self) -> Vec<TaskInput> {
        let mut v = vec![
            TaskInput::File(self.raw_elf.clone()),
            TaskInput::File(self.cart_info_file.clone()),
        ];
        for (_, path) in &self.extra_sections { v.push(TaskInput::File(path.clone())); }
    ...
    fn outputs(&self) -> Vec<PathBuf> { vec![self.output.clone()] }
    fn run(&self) -> Result<(), BuildError> {
        let extra: Vec<(&str, &Path)> =
            self.extra_sections.iter().map(|(n, p)| (n.as_str(), p.as_path())).collect();
        finalise_cart(&self.objcopy, &self.raw_elf, &self.cart_info_file, &self.output, &extra, self.debug)
    }
}
...
pub fn run(
    ...
    let native_count = languages.iter().filter(|&&l| l != CartLanguage::Lua).count();
    ...
    if languages.contains(&CartLanguage::C) {
        ...
        if external_config.is_none() && collect_c_files(&c_src_dir)?.is_empty() {
            return Err(err(format!("no .c files found under {}", c_src_dir.display())));
        }
    }
    ...
    let lua_lib_defines: Vec<String> = if is_lua {
        vec!["-DBLYT_LUA_I32_F64=1".to_string(), "-DLUA_USE_LONGJMP=1".to_string()]
    } else {
        vec![]
    };
    ...
    let build_dir = if is_lua {
        ...
        project_dir.join("build/game").join(&cfg.language).join(variant)
    } else {
    ...
    write_if_changed(&ld_script, if is_lua { HYBRID_LUA_LINKER_SCRIPT } else { LINKER_SCRIPT })?;
    ...
    tasks.push(Box::new(make_c_task(
        project_dir, entry_stub_src.clone(), build_dir.clone(), &clang, &sdk_include,
        vec![], vec![], opt_c_flags.clone(), "compile_c",
    )));
    tasks.push(Box::new(make_c_task(
        project_dir, interp_src.clone(), build_dir.clone(), &clang, &sdk_include,
        vec![], vec![], opt_c_flags.clone(), "compile_c",
    )));
    ...
    if is_lua {
        ...
        tasks.push(Box::new(make_c_task(
            project_dir, data_c, build_dir.clone(), &clang, &sdk_include,
            vec![], vec![], vec![], "compile_c",
        )));
        // Glue C is produced by CodegenTask; compile it here.
        tasks.push(Box::new(make_c_task(
            project_dir, glue_src, build_dir.clone(), &clang, &sdk_include,
            vec![], lua_lib_defines.clone(), vec![], "compile_c",
        )));
    }

    // Discover lib include paths (known from directory structure, no build needed).
    let lib_names = if external_config.is_none() { discover_libraries(project_dir)? } else { Vec::new() };
    ...
    for name in &lib_names {
        ...
        let include_path = {
            ...
            if with_include.is_dir() { with_include } else { src_dir.clone() }
        };
        ...
        if c_files.is_empty() && cpp_files.is_empty() {
            return Err(err(format!("library {name}: no source files found under {}", src_dir.display())));
        }
        let mut lib_obj_files: Vec<PathBuf> = Vec::new();
        for src in c_files {
            let task = make_c_task(
                project_dir, src, lib_build_dir.clone(), &clang, &sdk_include,
                vec![include_path.clone()], lua_lib_defines.clone(), opt_c_flags.clone(),
        ...
        for src in cpp_files {
            ...
            let task = make_cpp_task(
                project_dir, src, lib_build_dir.clone(), &clangpp, &sdk_include,
                libcxx_include, vec![include_path.clone()], opt_c_flags.clone(),
    ...
    // Game C sources.
    if languages.contains(&CartLanguage::C) {
        let extra_defines = if is_lua { lua_lib_defines.clone() } else { vec![] };
        for src in collect_c_files(&project_dir.join("src/game/c"))? {
            tasks.push(Box::new(make_c_task(
                project_dir, src, build_dir.clone(), &clang, &sdk_include,
                c_include_paths.clone(), extra_defines.clone(), opt_c_flags.clone(),
                "compile_c",
            )));
        }
    }
    ...
    let libcxx_archive = if languages.contains(&CartLanguage::Cpp) {
        ...
        for src in collect_cpp_files(&project_dir.join("src/game/c++"))? {
            tasks.push(Box::new(make_cpp_task(
                project_dir, src, build_dir.clone(), &clangpp, &sdk_include,
                libcxx_include.clone(), c_include_paths.clone(), opt_c_flags.clone(),
                "compile_cpp",
            )));
        }
        let libcxxabi_a = lib_dir.join("libc++abi.a");
        Some((libc_a, if libcxxabi_a.exists() { Some(libcxxabi_a) } else { None }))
    } else {
        None
    };
    if let Some((libc_a, libcxxabi_a)) = libcxx_archive {
        lib_archives.push(libc_a);
        if let Some(a) = libcxxabi_a { lib_archives.push(a); }
    }
    ...
    let external_output = if let Some(ref cfg) = external_config {
        let sdk_bin = lib_dir.parent().map(|p| p.join("bin")).unwrap_or_else(|| {
            sdk_include.parent().map(|p| p.join("bin")).unwrap_or_else(|| PathBuf::from("bin"))
        });
        ...
        tasks.push(Box::new(ExternalCompileTask {
            ...
            config: ExternalCompileConfig {
                ...
                output_type: if is_obj { ExternalOutputType::Object } else { ExternalOutputType::Archive },
            },
            project_dir: project_dir.to_path_buf(),
            build_dir: build_dir.clone(),
    ...
    if let Some((path, is_obj)) = external_output {
        if is_obj { obj_files.push(path); } else { final_lib_archives.push(path); }
    }
    ...
    let lua_bytecode_path = if is_lua { Some(build_dir.join("bytecode.luac")) } else { None };
    ...
    if buffers_present {
        extra_sections.push((".cart.layouts".to_string(), build_dir.join("cart.layouts.bin")));
    }
...
pub fn build_single_lib(
    ...
    for src in c_files {
        let task = make_c_task(
            project_dir, src, lib_build_dir.clone(), &clang, &sdk_include,
            vec![include_path.clone()], vec![], vec![],
    ...
    for src in cpp_files {
        ...
        let task = make_cpp_task(
            project_dir, src, lib_build_dir.clone(), &clangpp, &sdk_include,
            libcxx_include, vec![include_path.clone()], vec![],
