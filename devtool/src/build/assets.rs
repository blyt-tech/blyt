...
/// and `blyt/lua/cart_resources.lua` (Lua carts).
...
impl Task for GenerateResourceHeadersTask {
    ...
    fn label(&self) -> String {
        "generate cart_resources.{h,lua}".to_string()
    }
    fn inputs(&self) -> Vec<TaskInput> {
        vec![
    ...
    fn outputs(&self) -> Vec<PathBuf> {
        vec![self.c_output.clone(), self.lua_output.clone()]
    }
    fn run(&self) -> Result<(), BuildError> {
        if let Some(parent) = self.c_output.parent() {
            fs::create_dir_all(parent)?;
        }
        if let Some(parent) = self.lua_output.parent() {
            fs::create_dir_all(parent)?;
        }
